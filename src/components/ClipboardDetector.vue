  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="insertCode" ref="confirmButton"
          >确定</el-button
        >
      </span>
    </template>
...
const checkClipboard = async () => {
  try {
    ...
    while ((jsMatch = jsRegex.exec(text)) !== null) {
      jsContent +=
        (jsMatch[1] || jsMatch[2] ? (jsMatch[1] || jsMatch[2]).trim() : '') +
        '\n\n'
    }
...
const insertCode = async () => {
  try {
    // 从HTML内容中提取标题作为名称
    const title = await extractTitleFromHTML(
      detectedCode.value.HTML.content || ''
    )
